...
| **P22** | Performance | Ανεξέλεγκτο frame loop & ενεργειακή σπατάλη | 🟡 Μεσαία | Adaptive Variable Refresh Rate (VRR) & On-Demand Repainting. |
| **P23** | Theming | Στατική παλέτα & έλλειψη προσβασιμότητας | 🟡 Μεσαία | Dual-Mode Theme Engine & System Theme Synchronization. |
| **P24** | UX/Designer | Τυφλό δεξί κλικ & αποσυγχρονισμός επιλογών | 🟡 Μεσαία | Hit-Test First Context Dispatcher & Unified Selection. |
| **P25** | Legal/Tax | myDATA & ευρωπαϊκή φορολογική συμμόρφωση | 🔴 Υψηλή | Operational Voucher Separation + Merkle Audit Ledger. |
| **P26** | Runtime | Ευπάθεια συνδέσεων Visual Flows με τη βάση | 🔴 Υψηλή | Bi-Directional Event Binding Bus & Form State Ingestion. |
| **P27** | Architecture | Πολυκαταστηματική απομόνωση χωρίς database bloat | 🔴 Υψηλή | Hierarchical Tenant Partitioning with Quota Enforcers. |
| **P28** | Mobile UX | Επικάλυψη πεδίων από εικονικό πληκτρολόγιο | 🟡 Μεσαία | Focus-Driven Scroll Cushion & 44pt Touch Targets. |
...
## Τομέας ΣΤ: UI/UX Engine, Variable Refresh Rate & Theming (P22 – P24)

### P22: Ανεξέλεγκτο Frame Loop, Ενεργειακή Σπατάλη & Θερμικό Throttling

#### Ρίζα του Προβλήματος
Σε άμεσα immediate-mode UI frameworks (`egui`/`eframe`), αν καλείται συνεχώς `ctx.request_repaint()`, η εφαρμογή εκτελεί έναν ατέρμονο βρόχο επανασχεδίασης σε μέγιστα δυνατά FPS (500+ FPS σε gaming monitors), καταναλώνοντας 100% ενός CPU/GPU πυρήνα, αδειάζοντας την μπαταρία σε φορητούς υπολογιστές και ανεβάζοντας θερμοκρασίες σε POS τερματικά.

#### Τεχνική Λύση: Adaptive Variable Refresh Rate (VRR) Engine
1. **Αρχιτεκτονική Προφίλ Ρυθμού Ανανέωσης (`VrrMode`):**
   * **Reactive / Power Saver (0% Idle):** Σε κατάσταση αδράνειας χωρίς κίνηση ποντικιού ή ενεργές ροές, η εφαρμογή αναστέλλει τον σχεδιασμό. Το `egui` ενεργοποιείται αυτόματα και ακαριαία μόνο κατά την είσοδο συμβάντων (mouse, key, socket).
   * **30 FPS (33.3ms Throttled):** Ελάχιστη κατανάλωση ενέργειας για αδύναμα tablets και mobile POS.
   * **60 FPS (16.6ms Standard):** Ισορροπημένη ομαλότητα κίνησης για desktop workstation.
   * **120 FPS / 144 FPS (ProMotion & High Refresh Displays):** Εξαιρετικά ομαλή κύλιση και drag-and-drop σχεδίαση.
   * **Continuous:** Απεριόριστη εκτέλεση αποκλειστικά για stress-testing και benchmarks.
2. **Δυναμικό Throttling:** Χρήση του `ctx.request_repaint_after(Duration::from_millis(ms))` αντί για άμεσο `ctx.request_repaint()`.

---

### P23: Στατική Σκούρα Παλέτα & Έλλειψη Προσβασιμότητας Φωτισμού

#### Ρίζα του Προβλήματος
Η προηγούμενη έκδοση είχε σταθερά κωδικοποιημένες σταθερές χρωμάτων σκοτεινού θέματος (`Color32::from_rgb(...)`), εμποδίζοντας την προσαρμογή σε φωτεινά γραφεία καταστημάτων, εξωτερικούς χώρους συνεργείων με έντονο ήλιο, και αγνοώντας την επιλογή Light/Dark mode του λειτουργικού συστήματος (Windows/macOS).

#### Τεχνική Λύση: Dual-Mode Theme Engine & Color Palette Presets
1. **Αυτόματος Συγχρονισμός Συστήματος (`ThemeMode`):**
   * `System`: Ανιχνεύει αυτόματα μέσω `ctx.system_theme()` εάν το λειτουργικό σύστημα είναι σε Dark ή Light mode και προσαρμόζει άμεσα την αντίθεση.
   * `Dark`: Sleek Obsidian & Linear-style Luxury Dark.
   * `Light`: Καθαρό, μινιμαλιστικό λευκό-γκρι φόντο υψηλής ευκρίνειας (High Contrast Minimalist).
2. **Παλέτες Έμφασης (Accent Customization):**
   * Linear Indigo (`#6366F1`)
   * Sapphire Blue (`#2563EB`)
   * Emerald Green (`#16A34A`)
   * Amber Gold (`#D97706`)
   * Rose Red (`#E11D48`)
   * Minimal Slate (`#475569`)
3. **WCAG AA Συμμόρφωση:** Εξασφάλιση αναλογίας αντίθεσης κειμένου $\ge 4.5:1$ σε όλα τα προφίλ.

---

### P24: Τυφλό Δεξί Κλικ & Αποσυγχρονισμός Επιλογής Canvas

#### Ρίζα του Προβλήματος
Όταν ο χρήστης έκανε δεξί κλικ σε ένα στοιχείο του καμβά χωρίς να το έχει προηγουμένως επιλέξει με αριστερό κλικ, το μενού ενεργειών εμφανιζόταν για το παλιό επιλεγμένο στοιχείο (ή για κανένα), προκαλώντας σύγχυση και λάθος διαγραφές/αντιγραφές.

#### Τεχνική Λύση: Hit-Test First Context Dispatcher & Unified Marquee State
1. **Προτεραιότητα Hit-Test κατά το Δεξί Κλικ:**
   * Πριν ανοίξει το context menu, το σύστημα εκτελεί αυτόματα hit-test στις συντεταγμένες του κέρσορα.
   * Εάν υπάρχει στοιχείο κάτω από το δεξί κλικ και δεν ανήκει ήδη στην τρέχουσα επιλογή, επιλέγεται άμεσα.
2. **Context-Aware Μενού Ενεργειών:**
   * **Μεμονωμένο Στοιχείο:** Αντιγραφή/Επικόλληση μεγέθους, Κλείδωμα, Απόκρυψη, Αντιγραφή (Ctrl+C), Διπλασιασμός (Ctrl+D), Διαγραφή.
   * **Πολλαπλή Επιλογή:** Ομαδοποίηση, Στοίχιση (Οριζόντια/Κατακόρυφη), Μαζική Διαγραφή.
   * **Κενός Καμβάς:** Γρήγορη Εισαγωγή (Smart Box, Κείμενο, Κουμπί, Πίνακας, KPI Card, Φόρμα), Επιλογή Όλων (Ctrl+A), Επικόλληση, Επαναφορά Zoom (Fit Viewport).

---

## Τομέας Ζ: Runtime Wiring, Multi-Store Isolation & Fiscal Law (P25 – P28)

### P25: Φορολογική Συμμόρφωση (myDATA, KassenSichV, NF525) & Διαχωρισμός Ευθύνης

#### Ρίζα του Προβλήματος
Η προσπάθεια ενσωμάτωσης τοπικών φορολογικών ταμειακών μηχανισμών απευθείας στον πυρήνα μιας διεθνούς πλατφόρμας δημιουργεί τεράστιο νομικό κόστος, συνεχείς αλλαγές νομοθεσίας (π.χ. ΑΑΔΕ myDATA στην Ελλάδα, TSE στη Γερμανία) και κίνδυνο νομικής ευθύνης του δημιουργού λογισμικού.

#### Τεχνική Λύση: Operational Voucher Separation + Merkle Audit Ledger
1. **Διαχωρισμός Εντολής Παραλαβής vs Φορολογικού Στοιχείου:**
   * Το Δελτίο Παραλαβής / Επισκευής (Intake Order) αποτελεί **Εσωτερικό Επιχειρησιακό Έγγραφο (Work Order)**.
   * Δεν υπόκειται σε υποχρέωση απευθείας διαβίβασης myDATA κατά την παραλαβή της συσκευής.
2. **Κρυπτογραφικό Merkle Ledger:**
   * Κάθε συναλλαγή και αλλαγή κατάστασης αποθηκεύεται σε αμετάβλητη αλυσίδα κατακερματισμού (Merkle chain SHA-256).
   * Παρέχει 100% αδιάβλητη απόδειξη εσωτερικού ελέγχου (tamper-evident audit log) για οποιονδήποτε φορολογικό ή δικαστικό έλεγχο.
3. **Ανοιχτό Webhook Gateway:**
   * Κατά την παράδοση και πληρωμή, το σύστημα εκπέμπει ένα τυποποιημένο event `TicketCompleted` προς εξωτερικούς πιστοποιημένους παρόχους ηλεκτρονικής τιμολόγησης (π.χ. Epsilon Net, Softone, myDATA ERP).

---

### P26: Ευπάθεια Συνδέσεων (Micro-Connections) Μεταξύ Visual Flows & Βάσης Δεδομένων

#### Ρίζα του Προβλήματος
Κατά τη σχεδίαση declarative φορμών, εάν αλλάξει το όνομα ενός πεδίου ή του κουμπιού, οι συνδέσεις με τις ροές αυτοματισμού (flows) και τα SQLite queries συχνά "σπάνε" σιωπηρά χωρίς ο χρήστης να το αντιληφθεί.

#### Τεχνική Λύση: Bi-Directional Event Binding Bus & Form State Ingestion
1. **Δυναμική Επίλυση Τιμών (Dynamic Field Resolution):**
   * Στο Play mode, ο εκτελεστής ροών αντλεί δυναμικά τις τιμές από το `form_state` κάνοντας resolve τόσο βάσει `bound_field` όσο και βάσει `node_id` ή `name`.
2. **Visual DAG Validation:**
   * Ο Flow Builder ελέγχει σε πραγματικό χρόνο εάν τα IDs των ενεργειών υπάρχουν στο active scene graph και επισημαίνει με προειδοποιητικό badge τυχόν ορφανές συνδέσεις.
3. **Conditional Branch Gating:**
   * Οι κόμβοι συνθηκών (`Condition`) αποτρέπουν την εκτέλεση άκυρων ή ελλιπών εγγραφών, επιστρέφοντας άμεση ανατροφοδότηση (toasts) στον χρήστη.

---

### P27: Πολυκαταστηματική Απομόνωση Δεδομένων χωρίς Υπερφόρτωση Βάσης

#### Ρίζα του Προβλήματος
Το μοντέλο "ξεχωριστή βάση δεδομένων ανά υποκατάστημα" καθιστά τον κεντρικό έλεγχο από το HQ εξαιρετικά δυσκίνητο, ενώ το μοντέλο "κοινός πίνακας χωρίς ιεραρχία" κινδυνεύει από διαρροή δεδομένων μεταξύ καταστημάτων.

#### Τεχνική Λύση: Hierarchical Tenant Partitioning with Quota Enforcers
1. **4-Tier Data Hierarchy:** `Enterprise` $\rightarrow$ `Store` $\rightarrow$ `Department` $\rightarrow$ `User`.
2. **Foreign Key Strict Partitioning:** Κάθε εγγραφή περιλαμβάνει `enterprise_id` και `store_id`.
3. **Επιβολή Quota στο Επίπεδο της Βάσης:**
   * Κατά την ανάθεση θέσης (provisioning), η SQLite ρουτίνα επικυρώνει ότι `active_seats < allocated_seats`.
   * Απορρίπτει άμεσα κάθε υπέρβαση, εξασφαλίζοντας την οικονομική ακεραιότητα του συνδρομητικού μοντέλου.

---

### P28: Επικάλυψη Πεδίων Εισαγωγής από το Εικονικό Πληκτρολόγιο σε Mobile POS

#### Ρίζα του Προβλήματος
Σε touch οθόνες (Android/iOS/Windows Tablets), όταν ο χειριστής πατάει σε ένα πεδίο στο κάτω μισό της οθόνης, το εικονικό πληκτρολόγιο που ανοίγει καλύπτει το πεδίο, εμποδίζοντας τον χρήστη να δει τι πληκτρολογεί.

#### Τεχνική Λύση: Focus-Driven Scroll Cushion & 44pt Touch Targets
1. **Dynamic Scroll Cushion:**
   * Όλα τα πεδία εισαγωγής σε φόρμες διαθέτουν listener εστίασης (`response.has_focus()`).
   * Μόλις εστιαστεί ένα πεδίο, ενεργοποιείται αυτόματο `response.scroll_to_me(Some(egui::Align::Center))`.
2. **Επιβολή Ελάχιστου Touch Target (44pt):**
   * Εφαρμογή των κατευθυντήριων γραμμών Apple HIG & Google Material (`min_touch_target: 44.0pt`) σε mobile προφίλ, εξαλείφοντας τα άστοχα πατήματα σε περιβάλλον ταμείου.
