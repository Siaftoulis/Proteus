...
#### Καρτέλα 3: Devices & Run (Προσομοιωτής & Εξαγωγή Πακέτου)
...
* **Export Engine (`.pr` Package)**:
  * Συμπίεση σκηνών, σχημάτων βάσης, workflows και προτύπων αποδείξεων.
  * Κρυπτογραφική υπογραφή (Ed25519) του πακέτου με metadata (Όνομα, Έκδοση, Δημιουργός).

---

## 3. Πυλώνας 2: Proteus Client (The Shop Counter Runtime)

### 3.1 Σκοπός & Κοινό-Στόχος
* **Χρήστες**: Ιδιοκτήτες επισκευαστικών κέντρων, συνεργείων, τεχνικοί πάγκου, υπάλληλοι ταμείου.
* **Αποστολή**: Καθημερινή εκτέλεση παραλαβών, διαχείριση ροής εργασιών (pipeline), έκδοση δελτίων σε θερμικό εκτυπωτή και τοπική αποθήκευση.
...
### 3.2 Δομή Οθονών & Ροή Εργασιών (Workflow Screens)

```
+-----------------------------------------------------------------------------------------+
| [PROTEUS BOS]   [⚡ Νέα Παραλαβή]   [📋 Ροή Επισκευών]   [⚙ Ρυθμίσεις]   | ● 100% Offline |
+-----------------------------------------------------------------------------------------+
```
...
#### Ενότητα 3: Πύλη Αδειών & Χρεώσεων (Licensing & Checkout)
* **Merchant of Record (Paddle / Lemon Squeezy)**:
  * Αυτόματη έκδοση τιμολογίων, διαχείριση ΦΠΑ ανά κράτος μέλος της Ε.Ε.
  * Συνδρομή Core: 7.99€ / μήνα ανά κατάστημα.
* **Έκδοση Κρυπτογραφικής Άδειας**:
  * Ασύμμετρη υπογραφή Ed25519 με μοναδικό Machine Fingerprint του καταστήματος.
  * Έκδοση Offline Lease Token 30 ημερών (επιτρέπει πλήρη λειτουργία ακόμα κι αν κοπεί το internet για 1 μήνα).
* **Self-Service Device Management**:
  * Ο ιδιοκτήτης μπορεί να μεταφέρει την άδεια σε νέο υπολογιστή (deactivate παλιό $\rightarrow$ activate νέο) με 1 κλικ.

#### Ενότητα 4: Πύλη Designers & In-Platform Escrow (Creator Economy)
* **Sandbox Επαλήθευσης (Verification Pipeline)**:
  * Αυτόματος έλεγχος ασφαλείας των ανεβασμένων `.pr` πακέτων:
    * Εξασφάλιση ότι τα migrations είναι μόνο additive (`CREATE TABLE`, `ALTER TABLE ADD COLUMN`).
    * Αποκλεισμός επικίνδυνων queries (`DROP`, `DELETE`).
* **Μοντέλο Εσόδων (Creator-First & Anti-Rent-Seeking)**:
  * **85% στον Δημιουργό (Designer)** / **15% στην Πλατφόρμα (Proteus)**:
    * Το 70/30 απορρίπτεται ρητά ως παρωχημένο "App Store Tax".
    * Από το 15% της πλατφόρμας, περίπου ~5% καλύπτει τα τέλη του Merchant of Record (Paddle/Stripe), αφήνοντας καθαρό ~10% για τη συντήρηση του marketplace και του escrow.
    * Ο designer λαμβάνει το 85% της αξίας του πακέτου, λειτουργώντας ως παγκόσμιος μαγνήτης για ταλέντο.
  * **In-Platform Escrow για Custom Συμβόλαια**:
    * **90% στον Designer / 10% στο Proteus** για εξατομικευμένες αναθέσεις από συνεργεία και καταστήματα.
* **Value-for-Money για τις Επιχειρήσεις**:
  * Συνδρομή Client: Μόνο **7.99€ / μήνα** (απόσβεση από την πρώτη μέρα).
  * Αγορά Προτύπων `.pr`: Εφάπαξ αγορά (One-time buy, π.χ. 19€–39€) χωρίς επαναλαμβανόμενα κόστη.
* **Custom Bespoke Contracts & Escrow**:
  * Κατάλογος Certified Designers όπου ιδιοκτήτες συνεργείων ζητούν εξατομικευμένη παραμετροποίηση.
  * In-platform Escrow: Ο πελάτης δεσμεύει το ποσό (π.χ. 250€) $\rightarrow$ Ο designer παραδίδει το `.pr` πακέτο $\rightarrow$ Ο πελάτης το εγκρίνει $\rightarrow$ Αποδέσμευση πληρωμής.
